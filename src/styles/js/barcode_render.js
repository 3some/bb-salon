...
var object_canvas;
      function render_barcode(element,id,style){   
        object_canvas = element;
		settings['output'] = style;
        if (style == 'canvas'){
          clearCanvas();
          $(element).hide();
          $(element).show().barcode(id, btype, settings);
        } else {
          $(element).hide();
          $(element).html("").show().barcode(id, btype, settings);
        }
      }
      
      function clearCanvas(){
        var canvas = $(object_canvas).get(0);
        var ctx = canvas.getContext('2d');
        ctx.lineWidth = 1;
        ctx.lineCap = 'butt';
